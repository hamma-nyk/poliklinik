    ...
    <x-slot name="header">
        <div class="flex flex-col md:flex-row justify-between items-center">
            <div>
                ...
                <p class="text-sm text-slate-500 mt-1">Kelola Akun Pengguna & Hak Akses Aplikasi</p>
            </div>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="bg-white p-4 rounded-2xl shadow-sm border border-slate-200 flex flex-col md:flex-row justify-between items-center gap-4">
                ...
                <div class="w-full md:w-auto">
                    <a href="{{ route('users.create') }}" class="inline-flex justify-center items-center px-5 py-2.5 bg-slate-900 text-white text-sm font-bold rounded-xl shadow-lg shadow-slate-800/30 hover:bg-slate-800 hover:scale-105 transition-all duration-200 w-full md:w-auto">
                    ...
                    <table class="min-w-full divide-y divide-slate-100">
                        <thead>
                            <tr class="bg-slate-50/50">
                                <th class="px-6 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">User Profile</th>
                                <th class="px-6 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">Hak Akses (Permissions)</th>
                            ...
                            <tr class="hover:bg-slate-50/80 transition duration-150 group">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center">
                                        <div class="flex-shrink-0 h-10 w-10 rounded-full bg-slate-200 flex items-center justify-center text-slate-600 font-bold text-sm shadow-sm border border-white">
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex flex-wrap gap-1.5">
                                        @forelse($user->permissions as $perm)
                                            <span class="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-medium bg-blue-50 text-blue-700 border border-blue-100 uppercase tracking-wide">
                                                {{ str_replace('_', ' ', $perm->name) }}
                                            </span>
                                        @empty
                                            <span class="text-xs text-slate-400 italic">User Biasa (No Special Access)</span>
                                        @endforelse
                                    </div>
                                </td>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    ...
                                    @else
                                        <div class="flex justify-end gap-2">
                                            <a href="{{ route('users.edit', $user) }}" class="text-slate-400 hover:text-blue-600 transition-colors p-2 rounded-lg hover:bg-blue-50" title="Edit User">
                                            ...
                                            <form action="{{ route('users.destroy', $user) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus user ini?');">
                                                @csrf @method('DELETE')
                                                <button type="submit" class="text-slate-400 hover:text-red-600 transition-colors p-2 rounded-lg hover:bg-red-50" title="Hapus User">
                            ...
                            <tr>
                                <td colspan="3" class="px-6 py-12 text-center">
                                    <div class="flex flex-col items-center justify-center">
                                        ...
                                        <h3 class="text-slate-500 font-medium">Belum ada user tambahan</h3>
                                        <p class="text-slate-400 text-sm mt-1">Tambahkan user baru untuk mengelola aplikasi.</p>
                                    </div>
                                </td>
                            </tr>
                            @endforelse

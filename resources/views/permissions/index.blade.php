    ...
    <x-slot name="header">
        <div class="flex flex-col md:flex-row justify-between items-center gap-4 transition-all duration-300">
            ...
            <div class="hidden md:flex items-center text-sm text-slate-500 dark:text-slate-400">
                ...
                <span class="font-semibold text-slate-900 dark:text-slate-50">Permissions</span>
            </div>
        </div>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="rounded-xl border border-slate-200 bg-white text-slate-950 shadow dark:border-slate-800 dark:bg-slate-950 dark:text-slate-50 p-6 flex flex-col md:flex-row justify-between items-center gap-4 transition-all">
                ...
                <div class="w-full md:w-auto">
                    <a href="{{ route('permissions.create') }}" class="inline-flex items-center justify-center whitespace-nowrap rounded-md text-sm font-medium transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-slate-950 disabled:pointer-events-none disabled:opacity-50 bg-slate-900 text-slate-50 shadow hover:bg-slate-900/90 h-9 px-4 py-2 dark:bg-slate-50 dark:text-slate-900 dark:hover:bg-slate-50/90 w-full md:w-auto">
                    ...
                    <table class="w-full caption-bottom text-sm">
                        <thead class="[&_tr]:border-b">
                            <tr class="border-b transition-colors hover:bg-slate-100/50 dark:border-slate-800 dark:hover:bg-slate-800/50">
                                <th class="h-12 px-4 text-left align-middle font-medium text-slate-500 dark:text-slate-400">Nama Permission</th>
                                <th class="h-12 px-4 text-left align-middle font-medium text-slate-500 dark:text-slate-400">Guard</th>
                                <th class="h-12 px-4 text-left align-middle font-medium text-slate-500 dark:text-slate-400">Dibuat Pada</th>
                            ...
                            <tr class="border-b transition-colors hover:bg-slate-100/50 dark:border-slate-800 dark:hover:bg-slate-800/50">
                                <td class="p-4 align-middle whitespace-nowrap">
                                    <div class="flex items-center">
                                        ...
                                        <div class="font-medium text-slate-800 dark:text-slate-100">{{ $permission->name }}</div>
                                    </div>
                                </td>
                                ...
                                <td class="p-4 align-middle whitespace-nowrap">
                                    ...
                                    <div class="text-xs text-slate-500 dark:text-slate-400">{{ $permission->created_at->format('H:i') }} WIB</div>
                                </td>
                                ...
                                <td class="p-4 align-middle whitespace-nowrap text-right text-sm font-medium">
                                    <div class="flex items-center justify-end gap-2">
                                        ...
                                        <form action="{{ route('permissions.destroy', $permission->id) }}" method="POST" onsubmit="return confirm('Hapus permission ini? Dapat mempengaruhi akses user.');" class="inline">
                                            @csrf @method('DELETE')
                                            <button type="submit" class="inline-flex items-center justify-center whitespace-nowrap rounded-md text-sm font-medium transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-slate-950 disabled:pointer-events-none disabled:opacity-50 hover:bg-red-100 hover:text-red-600 h-8 w-8 bg-transparent dark:hover:bg-red-900/50 dark:hover:text-red-500" title="Hapus">

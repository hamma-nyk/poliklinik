    ...
    <x-slot name="header">
        <div class="flex flex-col md:flex-row justify-between items-center gap-4">
            ...
            <div class="flex items-center text-sm text-slate-500 dark:text-slate-400">
                ...
                <span class="font-semibold text-slate-900 dark:text-slate-50">Kritis</span>
            </div>
        </div>
    </x-slot>
    ...
    <div class="py-6 flex-1 space-y-4">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="flex flex-col md:flex-row justify-between items-stretch md:items-center gap-4">
                <div class="flex border border-slate-200 bg-white text-slate-950 shadow dark:border-slate-800 dark:bg-slate-950 dark:text-slate-50 p-4 rounded-xl">
                    <div class="flex items-center">
                        <svg class="w-5 h-5 text-slate-500 dark:text-slate-400 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"></path></svg>
                        <div>
                            <p class="font-medium text-sm">Ambang Batas Peringatan</p>
                            <p class="text-xs text-slate-500 dark:text-slate-400">Menampilkan item dengan sisa stok &le; <span class="font-semibold">{{ $limit }}</span> unit.</p>
                        </div>
                    </div>
                </div>
                    ...
                    <table class="w-full caption-bottom text-sm">
                        <thead class="[&_tr]:border-b">
                            <tr class="border-b transition-colors hover:bg-slate-100/50 dark:border-slate-800 dark:hover:bg-slate-800/50">
                                <th class="h-12 px-4 text-center align-middle font-medium text-slate-500 dark:text-slate-400 w-20">No</th>
                                <th class="h-12 px-4 text-left align-middle font-medium text-slate-500 dark:text-slate-400">Informasi Item</th>
                                <th class="h-12 px-4 text-center align-middle font-medium text-slate-500 dark:text-slate-400">Satuan</th>
                                <th class="h-12 px-4 text-center align-middle font-medium text-slate-500 dark:text-slate-400">Sisa Stok</th>
                                <th class="h-12 px-4 text-center align-middle font-medium text-slate-500 dark:text-slate-400">Tingkat Urgensi</th>
                            </tr>
                        </thead>
                        <tbody class="[&_tr:last-child]:border-0">
                            @forelse($data as $index => $row)
                            <tr class="border-b transition-colors hover:bg-slate-100/50 dark:border-slate-800 dark:hover:bg-slate-800/50">
                                <td class="p-4 align-middle text-center text-slate-500 dark:text-slate-400">{{ $index + 1 }}</td>
                                <td class="p-4 align-middle">
                                    <div class="font-medium text-slate-900 dark:text-slate-100">{{ $row->name }}</div>
                                    <div class="text-xs text-slate-500 dark:text-slate-400 mt-0.5 uppercase">{{ $row->code }}</div>
                                </td>
                                <td class="p-4 align-middle text-center">
                                    <span class="px-2.5 py-1 bg-slate-100 dark:bg-slate-800 rounded-md text-xs font-medium text-slate-600 dark:text-slate-300 border border-slate-200 dark:border-slate-700">
                                        {{ $row->unit }}
                                    </span>
                                </td>
                                <td class="p-4 align-middle text-center">
                            ...
                            <tr>
                                <td colspan="5" class="p-4 align-middle text-center py-24">
                                    <div class="flex flex-col items-center justify-center">
                                        <h3 class="text-slate-900 dark:text-slate-100 font-medium text-lg tracking-tight">Stok Logistik Aman</h3>

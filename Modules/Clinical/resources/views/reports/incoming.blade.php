<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col md:flex-row justify-between items-center gap-4">
            ...
            <div class="flex items-center text-sm text-slate-500 dark:text-slate-400">
                ...
                <span class="font-semibold text-slate-900 dark:text-slate-50">Restock</span>
            </div>
        </div>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="rounded-xl border border-slate-200 bg-white text-slate-950 shadow dark:border-slate-800 dark:bg-slate-950 dark:text-slate-50 overflow-hidden">
                <div class="p-6 border-b border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                    ...
                    <span class="text-xs text-slate-500 dark:text-slate-400">Data Restock Terkumpul</span>
                </div>
    ...
    <tr class="border-b transition-colors hover:bg-slate-100/50 dark:border-slate-800 dark:hover:bg-slate-800/50">
        ...
        <th class="h-12 px-4 text-center align-middle font-medium text-slate-500 dark:text-slate-400 w-48">
            Nomor Faktur
        </th>
        ...
        {{-- Harga Beli Satuan --}}
        <th class="h-12 px-4 text-right align-middle font-medium text-slate-500 dark:text-slate-400 w-48">
            Harga Beli (@)
        </th>
    </tr>
</thead>
    ...
    @forelse($data as $medicineId => $group)
        ...
        <tr class="border-b transition-colors hover:bg-slate-100/50 dark:border-slate-800 dark:hover:bg-slate-800/50 bg-slate-50/50 dark:bg-slate-900/40">
            <td colspan="3" class="p-4 align-middle">
                <div class="flex flex-col">
                    <span class="text-xs text-slate-500 dark:text-slate-400">Master Barang</span>
                    <span class="font-medium text-slate-900 dark:text-slate-100">
                        {{ $medicine->name ?? 'Obat Dihapus' }} 
                        <span class="text-slate-500 ml-2">[{{ $medicine->code ?? 'N/A' }}]</span>
                    </span>
                </div>
            </td>
            <td class="p-4 align-middle text-center">
                <span class="text-xs text-slate-500 dark:text-slate-400 block">Total Volume</span>
                <span class="font-medium text-slate-900 dark:text-slate-100">
                    {{ number_format($subtotalQty, 0, ',', '.') }} {{ $medicine->unit }}
                </span>
            </td>
            <td class="p-4 align-middle text-right">
                <span class="text-xs text-slate-500 dark:text-slate-400 block">Subtotal Investasi</span>
                <span class="font-medium text-slate-900 dark:text-slate-100">
        ...
        @foreach($group as $row)
            <tr class="border-b transition-colors hover:bg-slate-100/50 dark:border-slate-800 dark:hover:bg-slate-800/50">
                <td class="p-4 align-middle text-center text-slate-500 dark:text-slate-400">
                    {{ $loop->iteration }}
                </td>
                ...
                <td class="p-4 align-middle text-center">
                    <div class="text-xs text-slate-500 dark:text-slate-400">No. Faktur</div>
                    <div class="font-medium">
                ...
                <td class="p-4 align-middle text-center">
                    ...
                    <div class="text-xs text-slate-500 dark:text-slate-400">{{ $medicine->unit }}</div>
                </td>
                <td class="p-4 align-middle text-right">
    ...
    @empty
        <tr class="border-b transition-colors hover:bg-slate-100/50 dark:border-slate-800 dark:hover:bg-slate-800/50">
            <td colspan="5" class="p-4 align-middle text-center text-slate-500 dark:text-slate-400 py-10">
...
@if($data->isNotEmpty())
    <tfoot class="border-t border-slate-200 dark:border-slate-800 font-medium">
        <tr class="bg-slate-50 dark:bg-slate-900">
            <td colspan="4" class="p-4 align-middle text-right text-xs text-slate-500 dark:text-slate-400 uppercase">
                Total Nilai Investasi Pengadaan Seluruhnya
            </td>
            <td class="p-4 align-middle text-right">
                <div class="text-xs text-slate-500 dark:text-slate-400 mb-1">Grand Total</div>
                <div class="font-medium text-slate-900 dark:text-slate-50 text-base">

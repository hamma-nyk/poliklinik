<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-slate-800">{{ __('Laporan Obat Masuk (Restock)') }}</h2>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white p-4 rounded-xl shadow-sm border border-slate-200 mb-6">
                <form method="GET" action="{{ route('clinical.reports.incoming') }}" class="flex flex-col md:flex-row gap-4 items-end">
                    <div>
                ...
                <table class="w-full text-sm text-left">
                    <thead class="bg-slate-100 text-slate-500 uppercase font-bold">
                        <tr>
                            ...
                            <th class="px-6 py-3 text-right">Total Nilai (Rp)</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-slate-100">
                        ...
                        <tr class="hover:bg-slate-50">
                            ...
                            <td class="px-6 py-4 font-mono text-slate-600">{{ $row->medicine->code ?? '-' }}</td>
                            <td class="px-6 py-4 font-bold text-slate-800">{{ $row->medicine->name ?? 'Obat Dihapus' }}</td>
                            <td class="px-6 py-4 text-center">{{ $row->medicine->unit ?? '-' }}</td>
                            <td class="px-6 py-4 text-center">
                                <span class="bg-green-100 text-green-800 py-1 px-3 rounded-full font-bold">
                                    +{{ $row->total_qty }}
                                </span>
                            </td>
                        ...
                        @empty
                        <tr>
                            <td colspan="6" class="px-6 py-8 text-center text-slate-400 italic">Tidak ada transaksi obat masuk pada periode ini.</td>
                        </tr>
                        @endforelse
                    </tbody>
                </table>

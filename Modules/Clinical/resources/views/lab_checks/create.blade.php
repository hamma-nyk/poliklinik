    ...
    <x-slot name="header">
        <div class="flex flex-col md:flex-row justify-between items-center gap-4">
            <div>
                ...
                <p class="text-sm text-neutral-500 mt-1 dark:text-neutral-400">Pencatatan Point of Care Testing (POCT)</p>
            </div>
             <div class="hidden md:flex items-center text-sm text-neutral-500 mt-2 md:mt-0 dark:text-neutral-400">
                <span class="hover:text-neutral-900 dark:hover:text-neutral-50 cursor-pointer transition-colors"><a href="{{ route('clinical.lab.index') }}">Laboratorium</a></span>
                <svg class="w-4 h-4 mx-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path></svg>
                <span class="font-semibold text-neutral-900 dark:text-neutral-50">Cek</span>
            </div>
        </div>
    </x-slot>
...
@endphp
    ...
    <div class="relative">
        ...
        <div class="absolute inset-y-0 right-0 flex items-center pr-3 cursor-pointer">
            <button type="button" x-show="selectedId || search" @click="clearSelection()" class="text-neutral-400 hover:text-red-500 focus:outline-none">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
            </button>
            <button type="button" x-show="!selectedId && !search" @click="isOpen = !isOpen" class="text-neutral-400 focus:outline-none">
    ...
    <div x-show="isOpen"
        ...
        <template x-for="item in filteredOptions" :key="item.id">
            <div @click="selectOption(item)"
                ...
                <svg x-show="selectedId == item.id" class="w-4 h-4 text-neutral-900 dark:text-neutral-50" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg>
            </div>
        </template>
...
<div class="mb-10 rounded-md border border-neutral-200 bg-neutral-50 dark:bg-neutral-900 dark:border-neutral-800 p-6"
        ...
        <template x-for="(item, index) in items" :key="index">
            <div class="flex gap-3 items-start animate-fade-in">
                    ...
                    <div x-show="item.isOpen" 
                        ...
                        <template x-for="med in getFiltered(item.search)" :key="med.id">
                            <div @click="
                                ...
                                <svg x-show="item.medicine_id == med.id" class="w-4 h-4 text-neutral-900 dark:text-neutral-50" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M5 13l4 4L19 7"/></svg>
                            </div>
                        </template>
                        <div x-show="getFiltered(item.search).length === 0" class="px-4 py-2 text-xs text-neutral-400 italic">Alat tidak ditemukan.</div>
                    </div>
                </div>
                ...
                <button type="button" 
                        ...
                        class="mt-2 text-neutral-400 hover:text-red-500 transition-colors">
    ...
    <div x-show="items.length === 0" class="text-center py-4 text-xs text-neutral-400 italic">
    ...
    <div class="p-3 rounded-lg text-sm flex items-center transition-colors duration-300"
         :class="(docId || nurId) ? 'bg-green-50 text-green-700 border border-green-200' : 'bg-neutral-100 text-neutral-500 border border-neutral-200'">
        ...
        <span x-text="(docId || nurId) ? 'Pemeriksa sudah dipilih (Valid).' : 'Mohon pilih Dokter atau Perawat (Wajib).'"></span>
            ...
            <div class="mt-6 p-6 rounded-md border border-neutral-200 bg-neutral-50 dark:bg-neutral-900 dark:border-neutral-800 flex items-start gap-4">
                ...
                <div>
                    <h4 class="text-xs font-semibold uppercase tracking-widest text-neutral-500 dark:text-neutral-400 mb-1">Referensi Batas Normal (Dewasa)</h4>
                    <p class="text-[11px] text-neutral-500 dark:text-neutral-400 leading-relaxed font-medium italic">

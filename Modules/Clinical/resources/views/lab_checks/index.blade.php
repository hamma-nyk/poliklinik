<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col md:flex-row justify-between items-center gap-4">
            ...
            <div class="hidden md:flex items-center text-sm text-slate-500 mt-2 md:mt-0 dark:text-slate-400">
                ...
                <span class="font-semibold text-slate-900 dark:text-slate-50">Laboratorium</span>
            </div>
        </div>
    </x-slot>
    ...
    <div class="py-6 flex-1 space-y-4">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="rounded-xl border border-slate-200 bg-white text-slate-950 shadow dark:border-slate-800 dark:bg-slate-950 dark:text-slate-50 p-6 flex flex-col md:flex-row justify-between items-center gap-4">
                ...
                <div class="w-full md:w-auto">
                    <a href="{{ route('clinical.lab.create') }}" class="inline-flex items-center justify-center whitespace-nowrap rounded-md text-sm font-medium transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-slate-950 disabled:pointer-events-none disabled:opacity-50 bg-slate-900 text-slate-50 shadow hover:bg-slate-900/90 h-9 px-4 py-2 dark:bg-slate-50 dark:text-slate-900 dark:hover:bg-slate-50/90 w-full md:w-auto">
            ...
            {{-- Table Container --}}
            <div class="rounded-xl border border-slate-200 bg-white text-slate-950 shadow dark:border-slate-800 dark:bg-slate-950 dark:text-slate-50 overflow-hidden sm:rounded-2xl">
                <div class="overflow-x-auto">
                    <table class="w-full caption-bottom text-sm divide-y divide-slate-100 dark:divide-slate-700">
                        <thead class="[&_tr]:border-b">
                            <tr class="border-b transition-colors hover:bg-slate-100/50 dark:border-slate-800 dark:hover:bg-slate-800/50">
                                <th class="h-12 px-4 text-left align-middle font-medium text-slate-500 dark:text-slate-400 uppercase tracking-widest text-[11px]">Waktu & Kode</th>
                                <th class="h-12 px-4 text-left align-middle font-medium text-slate-500 dark:text-slate-400 uppercase tracking-widest text-[11px]">Pasien</th>
                                <th class="h-12 px-4 text-center align-middle font-medium text-slate-500 dark:text-slate-400 uppercase tracking-widest text-[11px]">Gula Darah</th>
                                <th class="h-12 px-4 text-center align-middle font-medium text-slate-500 dark:text-slate-400 uppercase tracking-widest text-[11px]">Kolesterol</th>
                                <th class="h-12 px-4 text-center align-middle font-medium text-slate-500 dark:text-slate-400 uppercase tracking-widest text-[11px]">Asam Urat</th>
                                <th class="h-12 px-4 text-left align-middle font-medium text-slate-500 dark:text-slate-400 uppercase tracking-widest text-[11px]">Dokter</th>
                                <th class="h-12 px-4 text-left align-middle font-medium text-slate-500 dark:text-slate-400 uppercase tracking-widest text-[11px]">Perawat</th>
                                <th class="h-12 px-4 text-right align-middle font-medium text-slate-500 dark:text-slate-400 uppercase tracking-widest text-[11px]">Aksi</th>
                            </tr>
                        </thead>
                        <tbody class="[&_tr:last-child]:border-0 divide-y divide-slate-100 dark:divide-slate-700">
                            ...
                            <tr class="border-b transition-colors hover:bg-slate-100/50 dark:border-slate-800 dark:hover:bg-slate-800/50 group">
                                <td class="p-4 align-middle whitespace-nowrap">
                                    ...
                                    <span class="inline-flex items-center rounded-md border border-transparent bg-slate-100 text-slate-900 px-2.5 py-0.5 text-xs font-semibold dark:bg-slate-800 dark:text-slate-50 uppercase font-mono">
                                        {{ $chk->code ?? '-' }}
                                    </span>
                                </td>
                                ...
                                <td class="p-4 align-middle whitespace-nowrap">
                                    ...
                                    <div class="mt-2">
                                        @if($chk->patient->type == 'karyawan')
                                            <span class="inline-flex items-center rounded-md border border-transparent bg-slate-900 text-slate-50 px-2.5 py-0.5 text-xs font-semibold dark:bg-slate-50 dark:text-slate-900 uppercase tracking-tighter">KARYAWAN</span>
                                        @else
                                            <span class="inline-flex items-center rounded-md border border-transparent bg-slate-100 text-slate-900 px-2.5 py-0.5 text-xs font-semibold dark:bg-slate-800 dark:text-slate-50 uppercase tracking-tighter">UMUM</span>
                                        @endif
                                    </div>
                                </td>
                                ...
                                <td class="p-4 align-middle whitespace-nowrap">
                                    <div class="flex items-center">
                                        ...
                                        @else
                                        <div class="flex h-9 w-9 items-center justify-center rounded-md border border-slate-200 bg-slate-50 text-xs font-medium dark:border-slate-800 dark:bg-slate-900 mr-3 text-slate-700 dark:text-slate-300">
                                            {{ substr($name, 0, 2) }}
                                        </div>
                                        <div class="grid gap-0.5">
                                            <div class="font-medium text-slate-900 dark:text-slate-200">{{ $name }}</div>
                                            <div class="text-xs text-slate-500 dark:text-slate-400 uppercase tracking-widest font-bold">
                                                Dokter
                                            </div>
                                        </div>
                                        @endif

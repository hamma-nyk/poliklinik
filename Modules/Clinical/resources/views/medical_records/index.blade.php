    ...
    <x-slot name="header">
        <div class="flex flex-col md:flex-row justify-between items-center">
            ...
            <div class="hidden md:flex items-center text-sm text-slate-500 mt-2 md:mt-0 dark:text-slate-400">
                ...
                <span class="font-semibold text-slate-900 dark:text-slate-50">Medical Record</span>
            </div>
        </div>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="rounded-xl border border-slate-200 bg-white text-slate-950 shadow dark:border-slate-800 dark:bg-slate-950 dark:text-slate-50 p-6 flex flex-col md:flex-row justify-between items-center gap-4">
                ...
                <div class="w-full md:w-auto">
                    <a href="{{ route('clinical.records.create') }}" class="inline-flex items-center justify-center whitespace-nowrap rounded-md text-sm font-medium transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-slate-950 disabled:pointer-events-none disabled:opacity-50 bg-slate-900 text-slate-50 shadow hover:bg-slate-900/90 h-9 px-4 py-2 dark:bg-slate-50 dark:text-slate-900 dark:hover:bg-slate-50/90 w-full md:w-auto">
                    ...
                    <table class="w-full caption-bottom text-sm">
                        <thead class="[&_tr]:border-b">
                            <tr class="border-b transition-colors hover:bg-slate-100/50 dark:border-slate-800 dark:hover:bg-slate-800/50">
                                <th class="h-12 px-4 text-left align-middle font-medium text-slate-500 dark:text-slate-400">Waktu & No. RM</th>
                                <th class="h-12 px-4 text-left align-middle font-medium text-slate-500 dark:text-slate-400">Pasien</th>
                                <th class="h-12 px-4 text-left align-middle font-medium text-slate-500 dark:text-slate-400">Diagnosa (ICD-10)</th>
                        ...
                        <tbody class="[&_tr:last-child]:border-0">
                            ...
                            <tr class="border-b transition-colors hover:bg-slate-100/50 dark:border-slate-800 dark:hover:bg-slate-800/50">
                                ...
                                <td class="p-4 align-middle whitespace-nowrap">
                                    ...
                                    <div class="text-xs text-slate-500 dark:text-slate-400 mb-1.5 flex items-center">
                                        <svg class="w-3 h-3 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                                        {{ $rm->created_at->format('H:i') }} WIB
                                    </div>
                                    <span class="inline-flex items-center rounded-md border border-slate-200 bg-slate-50 text-slate-950 px-2.5 py-0.5 text-xs font-semibold dark:border-slate-800 dark:bg-slate-900 dark:text-slate-50">
                                        {{ $rm->code }}
                                    </span>
                                </td>
                                ...
                                {{-- Kolom 2 --}}
                                <td class="p-4 align-middle whitespace-nowrap">
                                    <div class="text-sm font-bold text-slate-900 dark:text-slate-100">{{ $rm->patient->name }}</div>
                                    ...
                                    <div class="mt-2">
                                        @if($rm->patient->type == 'karyawan')
                                            <span class="inline-flex items-center rounded-md border border-transparent bg-slate-100 text-slate-900 px-2.5 py-0.5 text-xs font-semibold dark:bg-slate-800 dark:text-slate-50 uppercase">KARYAWAN</span>
                                        @else
                                            <span class="inline-flex items-center rounded-md border border-slate-200 px-2.5 py-0.5 text-xs font-semibold text-slate-950 dark:border-slate-800 dark:text-slate-50 uppercase">UMUM</span>
                                        @endif
                                    </div>
                                </td>
                                ...
                                <td class="p-4 align-middle">
                                    @if($rm->diagnosis)
                                        <div class="text-sm font-bold text-slate-900 dark:text-slate-100">{{ $rm->diagnosis->name }}</div>
                                        <div class="inline-flex items-center rounded-md border px-2.5 py-0.5 text-xs font-semibold transition-colors border-slate-200 text-slate-950 dark:border-slate-800 dark:text-slate-50 mt-1">
                                            {{ $rm->diagnosis->code ?? '-' }}
                                        </div>
                                    @else
                                    ...
                                    <div class="text-xs text-slate-500 dark:text-slate-400 mt-2 truncate max-w-xs flex items-center italic" title="{{ $rm->keluhan_utama }}">
                                ...
                                <td class="p-4 align-middle whitespace-nowrap">
                                    <div class="flex items-center">
                                        ...
                                        @else
                                        <div class="flex h-9 w-9 items-center justify-center rounded-md border border-slate-200 bg-slate-50 text-xs font-medium dark:border-slate-800 dark:bg-slate-900 mr-3">
                                            {{ substr($name, 0, 2) }}
                                        </div>
                            ...
                            <tr>
                                <td colspan="6" class="p-4 align-middle text-center">
                                    <div class="flex flex-col items-center justify-center py-8">
                                        ...
                                        <h3 class="text-slate-500 dark:text-slate-400 font-medium">Belum ada rekam medis</h3>
                                        <p class="text-slate-400 dark:text-slate-500 text-sm mt-1">Klik tombol "Input Pemeriksaan" untuk mencatat diagnosa pasien baru.</p>
                                    </div>
                                </td>
                            </tr>
                            @endforelse

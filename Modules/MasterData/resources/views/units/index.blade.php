<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-neutral-800">{{ __('Master Unit (Sub Sub Bagian)') }}</h2>
    </x-slot>
    ...
    <div class="py-12" x-data="{ openImport: false }">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="flex justify-between items-center mb-6">
                ...
                <a href="{{ route('master.units.create') }}" class="bg-blue-600 text-white px-4 py-2 rounded-lg text-sm font-bold hover:bg-blue-700 shadow-sm">
                    + Tambah Unit
                </a>
            </div>
                ...
                <table class="w-full text-sm text-left">
                    ...
                    <tbody class="divide-y divide-neutral-100">
                        ...
                        <tr class="hover:bg-neutral-50">
                            <td class="px-6 py-4 font-mono font-bold text-neutral-700">{{ $unit->code }}</td>
                            <td class="px-6 py-4">{{ $unit->name }}</td>
                            <td class="px-6 py-4 text-right">
                                <a href="{{ route('master.units.edit', $unit->id) }}" class="text-blue-600 hover:underline font-bold mr-3">Edit</a>
                                <form action="{{ route('master.units.destroy', $unit->id) }}" method="POST" class="inline" onsubmit="return confirm('Hapus data ini?');">
                                    @csrf @method('DELETE')
                        ...
                        @empty
                        <tr>
                            <td colspan="3" class="px-6 py-6 text-center text-neutral-400 italic">Belum ada data unit.</td>
                        </tr>
                        @endforelse
                    </tbody>
                </table>
        ...
        <div x-show="openImport" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 backdrop-blur-sm" x-cloak>
            <div class="bg-white rounded-xl shadow-2xl w-full max-w-md p-6" @click.away="openImport = false">
                <h3 class="text-lg font-bold mb-4 text-neutral-800">Import Data Unit</h3>
                
                <form action="{{ route('master.units.import') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="mb-4">
                    ...
                    <div class="mb-6 bg-blue-50 p-4 rounded-lg text-xs text-blue-700 border border-blue-100">
                        ...
                        <div class="mt-3">
                            <a href="{{ route('master.units.template') }}" class="underline font-bold hover:text-blue-900">Download Template CSV</a>
                        </div>
                    </div>

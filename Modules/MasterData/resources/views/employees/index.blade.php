<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col md:flex-row justify-between items-center gap-4">
            <div>
                ...
                <p class="text-sm text-slate-500 dark:text-slate-400">Manajemen Database Sumber Daya Manusia (HR)</p>
            </div>
            <div class="hidden md:flex items-center text-sm text-slate-500 dark:text-slate-400">
                <span class="hover:text-slate-900 dark:hover:text-slate-50 cursor-pointer transition-colors">Master Data</span>
                <svg class="w-4 h-4 mx-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path></svg>
                <span class="font-semibold text-slate-900 dark:text-slate-50">Karyawan</span>
            </div>
        </div>
    </x-slot>
    ...
    <div class="py-6 flex-1 space-y-4">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            {{-- Action & Search Bar --}}
            <div class="rounded-xl border border-slate-200 bg-white text-slate-950 shadow dark:border-slate-800 dark:bg-slate-950 dark:text-slate-50 p-4 flex flex-col md:flex-row justify-between items-center gap-4">
                
               <form method="GET" class="w-full lg:w-auto flex flex-col sm:flex-row gap-3 items-center">
                    <div class="relative w-full sm:w-auto">
                        <select name="per_page" onchange="this.form.submit()" 
                                class="flex h-9 w-full sm:w-24 items-center justify-between whitespace-nowrap rounded-md border border-slate-200 bg-transparent px-3 py-1 text-sm shadow-sm ring-offset-white placeholder:text-slate-500 focus:outline-none focus:ring-1 focus:ring-slate-950 disabled:cursor-not-allowed disabled:opacity-50 dark:border-slate-800 dark:ring-offset-slate-950 dark:placeholder:text-slate-400 dark:focus:ring-slate-300">
                ...
                <div class="flex flex-col sm:flex-row gap-2 w-full md:w-auto">
                    ...
                    <form action="{{ route('master.employees.import') }}" method="POST" enctype="multipart/form-data" 
                          class="flex items-center gap-2 h-9 rounded-md border border-slate-200 bg-transparent px-2 text-sm shadow-sm dark:border-slate-800 w-full sm:w-auto">
                        @csrf
                        <input type="file" name="csv_file" class="block w-full text-xs text-slate-500 dark:text-slate-400 file:border-0 file:bg-transparent file:text-sm file:font-medium file:cursor-pointer"/>
                        <button type="submit" class="inline-flex items-center justify-center whitespace-nowrap rounded-md text-sm font-medium transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-slate-950 disabled:pointer-events-none disabled:opacity-50 border border-slate-200 bg-white shadow-sm hover:bg-slate-100 hover:text-slate-900 h-7 w-7 dark:border-slate-800 dark:bg-slate-950 dark:hover:bg-slate-800 dark:hover:text-slate-50" title="Upload CSV">
          ...
          onsubmit="document.getElementById('btn-sync-icon').classList.add('hidden'); document.getElementById('btn-sync-loading').classList.remove('hidden');">
                    ...
                    <form action="{{ route('master.employees.trim') }}" method="POST" class="w-full sm:w-auto" onsubmit="return confirm('Yakin ingin membersihkan spasi (TRIM) pada semua data karyawan? Proses ini aman dan tidak menghapus data.');">
                        @csrf
                        <button type="submit" class="inline-flex items-center justify-center whitespace-nowrap rounded-md text-sm font-medium transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-slate-950 disabled:pointer-events-none disabled:opacity-50 border border-slate-200 bg-white shadow-sm hover:bg-slate-100 hover:text-slate-900 h-9 px-4 py-2 dark:border-slate-800 dark:bg-slate-950 dark:hover:bg-slate-800 dark:hover:text-slate-50 w-full" title="Bersihkan spasi berlebih di database">
            ...
            {{-- Table Section --}}
            <div class="rounded-xl border border-slate-100 bg-white text-slate-950 shadow dark:border-slate-800 dark:bg-slate-950 dark:text-slate-50 overflow-hidden">
                    ...
                    <table class="w-full caption-bottom text-sm">
                        <thead class="[&_tr]:border-b">
                            <tr class="border-b border-slate-200 transition-colors hover:bg-slate-100/50 dark:border-slate-800 dark:hover:bg-slate-800/50">
                                <th class="h-12 px-4 text-left align-middle font-medium text-slate-500 dark:text-slate-400">Karyawan</th>
                                <th class="h-12 px-4 text-left align-middle font-medium text-slate-500 dark:text-slate-400">Bagian</th>
                                <th class="h-12 px-4 text-left align-middle font-medium text-slate-500 dark:text-slate-400">Sub Bagian</th>
                                <th class="h-12 px-4 text-center align-middle font-medium text-slate-500 dark:text-slate-400">Status</th>
                                <th class="h-12 px-4 text-center align-middle font-medium text-slate-500 dark:text-slate-400">ID System</th>
                            ...
                            <tr class="border-b border-slate-100 transition-colors hover:bg-slate-100/50 dark:border-slate-800 dark:hover:bg-slate-800/50">
                                <td class="p-4 align-middle">
                                    <div class="flex items-center gap-3">
                                        <div class="flex h-9 w-9 items-center justify-center rounded-md border border-slate-100 bg-slate-50 text-xs font-medium dark:border-slate-800 dark:bg-slate-900">
                                            {{ substr($emp->nama, 0, 1) }}{{ substr(strrchr($emp->nama, " "), 1, 1) }}
                                        </div>
                                        <div class="grid gap-0.5">
                                            <div class="font-medium">{{ $emp->nama }}</div>
                                            <div class="text-xs text-slate-500 dark:text-slate-400">{{ $emp->position->name ?? '-' }}</div>
                                            <div class="text-[10px] text-slate-400 dark:text-slate-500">NIK: {{ $emp->nik }}</div>
                                        </div>
                                    </div>
                                </td>
                                ...
                                <td class="p-4 align-middle text-center">
                                    ...
                                    @else
                                        <div class="inline-flex items-center rounded-md border border-transparent bg-slate-100 text-slate-500 px-2.5 py-0.5 text-xs font-semibold dark:bg-slate-800 dark:text-slate-400">

            ...
            <div class="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700">
                <form method="GET" action="{{ route('inventory.reports.stock_card') }}" class="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                    ...
                    <div class="md:col-span-2 space-y-1.5">
                        <label class="block text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest">Pilih Item Obat</label>
                    ...
                    <div class="space-y-1.5">
                        <label class="block text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest">Dari Tanggal</label>
                    ...
                    <div class="flex gap-2 items-end">
                        <div class="w-full space-y-1.5">
                            <label class="block text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest">Sampai Tanggal</label>
            ...
            <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden transition-all">
                ...
                <div class="p-8 border-b border-slate-100 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-800/50 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                    <div class="flex items-center">
                        ...
                        <div>
                            <h3 class="text-xl font-bold text-slate-800 dark:text-slate-100 tracking-tight">{{ $selectedMedicine->name }}</h3>
                    ...
                    <div class="bg-white dark:bg-slate-900 px-6 py-3 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-sm text-center md:text-right">
                        <span class="block text-[10px] text-slate-400 dark:text-slate-500 font-bold uppercase tracking-[0.2em] mb-1">Stok Saat Ini</span>
                        <span class="text-3xl font-bold text-blue-600 dark:text-blue-400 tabular-nums">{{ $selectedMedicine->current_stock }}</span>
                    </div>
                </div>
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-sm text-left">
                        <thead>
                            <tr class="bg-slate-100/50 dark:bg-slate-900/30 text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest">
                        ...
                        <tbody class="divide-y divide-slate-100 dark:divide-slate-700">
                            ...
                            {{-- Baris Stok Awal --}}
                            <tr class="bg-slate-50/80 dark:bg-slate-900/50 font-bold">
                            ...
                            @forelse($transactions as $item)
                                ...
                                <tr class="hover:bg-slate-50/80 dark:hover:bg-slate-700/40 transition-colors group">
                                    <td class="px-8 py-5 whitespace-nowrap">
                                        <div class="font-bold text-slate-800 dark:text-slate-100 uppercase">{{ \Carbon\Carbon::parse($item->transaction->transaction_date)->format('d M Y') }}</div>
                                    ...
                                    <td class="px-6 py-5">
                                        <div class="font-medium text-sm text-slate-700 dark:text-slate-200 tracking-tight">
                                            @if($item->transaction->medical_record_id)
                                                <span class="text-blue-600 font-bold text-base leading-none dark:text-blue-400">Resep Pasien</span>
                                            @elseif($item->transaction->notes && str_contains(strtolower($item->transaction->notes), 'pembelian'))
                                                <span class="text-emerald-600 font-bold text-base leading-none dark:text-emerald-400">Restock Pembelian</span>
                                            @else
...
<a href="{{ route('inventory.transactions.index') }}?search={{ $item->transaction->code }}" class="hover:text-amber-600 transition-colors">({{$item->transaction->code}})</a>
                                    ...
                                    <td class="px-6 py-5 text-center">
                                        @if($qtyIn > 0)
                                            <span class="text-md font-bold text-emerald-600 dark:text-emerald-400">+{{ $qtyIn }}</span>
                                        @else
                                    ...
                                    <td class="px-6 py-5 text-center">
                                        @if($qtyOut > 0)
                                            <span class="text-md font-bold text-rose-600 dark:text-rose-400">-{{ $qtyOut }}</span>
                                        @else
                                            <span class="text-slate-200 dark:text-slate-700">-</span>
                                        @endif
                                    </td>
                                    <td class="px-8 py-5 text-center bg-blue-50/30 dark:bg-blue-900/10">
                                        <span class="text-md font-bold text-slate-800 dark:text-slate-100 tabular-nums">{{ $currentBalance }}</span>
                                    </td>
                                </tr>
                            @empty
                                <tr>
                            ...
                            <tr class="bg-slate-100 font-bold border-t-2 border-slate-200">
                                <td colspan="2" class="px-6 py-3 text-right">
                                    TOTAL QTY <br>
                                    <span class="text-sm font-normal text-slate-500">Total Nilai Masuk: Rp {{ number_format($totalValueIn, 0, ',', '.') }}</span>
                                </td>
                                <td class="px-6 py-3 text-center">
                                    <div class="text-emerald-600 font-bold text-lg">{{ number_format($totalIn) }}</div>
                                </td>
                                <td class="px-6 py-3 font-bold text-lg text-center text-rose-600">{{ number_format($totalOut) }}</td>
                                <td class="px-6 py-3 font-bold text-lg text-center text-blue-800">{{ number_format($currentBalance) }}</td>
                            </tr>
            ...
            @else
                {{-- State Belum Pilih --}}
                <div class="flex flex-col items-center justify-center p-20 bg-white dark:bg-slate-800 border-2 border-dashed border-slate-200 dark:border-slate-700 rounded-2xl text-slate-400 transition-all">
                    ...
                    <h3 class="text-xl font-bold text-slate-500 dark:text-slate-400 uppercase tracking-tighter">Siap Meninjau Stok?</h3>

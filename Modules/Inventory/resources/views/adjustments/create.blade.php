<x-app-layout title="Buat Penyesuaian Stok">
    <x-slot name="header">
        <div class="flex flex-col md:flex-row justify-between items-center">
            ...
            <div class="hidden md:flex items-center text-sm text-slate-500 mt-2 md:mt-0 dark:text-slate-400">
                <span class="hover:text-slate-900 dark:hover:text-slate-50 cursor-pointer transition-colors"><a href="{{ route('inventory.adjustments.index') }}">Inventaris</a></span>
                <svg class="w-4 h-4 mx-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path></svg>
                <span class="font-semibold text-slate-900 dark:text-slate-50">Buat Adjustment</span>
            </div>
        </div>
    </x-slot>
    ...
    <div class="py-6 flex-1 space-y-4">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            ...
            <form action="{{ route('inventory.adjustments.store') }}" method="POST" 
                ...
                <div class="rounded-xl border border-slate-200 bg-white text-slate-950 shadow dark:border-slate-800 dark:bg-slate-950 dark:text-slate-50 p-6 space-y-8">
                    
                    {{-- Baris 1: Tanggal & Tipe --}}
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8 border-b border-slate-100 dark:border-slate-800 pb-8">
                        <div class="space-y-2">
                        ...
                        <div x-show="isOpen" 
                            ...
                            <template x-for="item in filteredItems" :key="item.id">
                                <div @click="selectItem(item)" 
                                    ...
                                    <div class="text-xs text-slate-500 dark:text-slate-400" x-text="item.code"></div>
                                </div>
                            </template>
                        ...
                        <div class="space-y-2">
                            ...
                            <div class="relative flex items-center">
                                <input type="number" name="quantity" min="1" placeholder="0" required
                                    class="flex h-9 w-full rounded-md border border-slate-200 bg-transparent px-3 py-1 text-sm shadow-sm transition-colors file:border-0 file:bg-transparent file:text-sm file:font-medium placeholder:text-slate-500 focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-slate-950 disabled:cursor-not-allowed disabled:opacity-50 dark:border-slate-800 dark:placeholder:text-slate-400 dark:focus-visible:ring-slate-300 pr-16 font-bold tabular-nums">
                                <div class="absolute right-4 flex items-center pointer-events-none border-l border-slate-200 dark:border-slate-800 pl-3 h-6 text-slate-500 dark:text-slate-400">
    ...
    {{-- Warning Info --}}
    <div class="flex items-center gap-3 text-xs text-slate-500 dark:text-slate-400 font-medium mr-auto">

    ...
    <x-slot name="header">
        <div class="flex flex-col md:flex-row justify-between items-center">
            <div>
                ...
                <p class="text-sm text-slate-500 mt-1">Inventaris & Stok Logistik Medis</p>
            </div>
            <div class="hidden md:flex items-center text-sm text-slate-500 mt-2 md:mt-0">
                ...
                <span class="font-semibold text-slate-700">Obat</span>
            </div>
        </div>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="bg-white p-4 rounded-2xl shadow-sm border border-slate-200 flex flex-col md:flex-row justify-between items-center gap-4">
                ...
                <div class="w-full md:w-auto">
                    <a href="{{ route('inventory.medicines.create') }}" class="inline-flex justify-center items-center px-5 py-2.5 bg-slate-900 text-white text-sm font-bold rounded-xl shadow-lg shadow-slate-800/30 hover:bg-slate-800 hover:scale-105 transition-all duration-200 w-full md:w-auto">
                    ...
                    <table class="min-w-full divide-y divide-slate-100">
                        <thead>
                            <tr class="bg-slate-50/50">
                                <th class="px-6 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">Detail Obat</th>
                                <th class="px-6 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">Satuan</th>
                                <th class="px-6 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">Harga Jual</th>
                            ...
                            <tr class="hover:bg-slate-50/80 transition duration-150 group">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center">
                                        <div class="flex-shrink-0 h-10 w-10 rounded-lg bg-gradient-to-br from-indigo-500 to-violet-600 flex items-center justify-center text-white font-bold text-sm shadow-md">
                                            {{ substr($medicine->name, 0, 1) }}
                                        </div>
                                        <div class="ml-4">
                                            <div class="text-sm font-bold text-slate-800 group-hover:text-indigo-600 transition-colors">{{ $medicine->name }}</div>
                                            <div class="text-xs font-mono text-slate-400 mt-0.5">{{ $medicine->code }}</div>
                                        </div>
                                    </div>
                                </td>

                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm text-slate-600 font-medium bg-slate-100 px-2 py-1 rounded inline-block">
                                        {{ $medicine->unit }}
                                    </div>
                                </td>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-center">
                                    @if($medicine->current_stock <= 5)
                                        <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-bold bg-red-100 text-red-700 border border-red-200 animate-pulse">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <div class="flex justify-end gap-2">
                                        <a href="{{ route('inventory.medicines.edit', $medicine->id) }}" 
                                           class="text-slate-400 hover:text-indigo-600 transition-colors p-2 rounded-lg hover:bg-indigo-50" title="Edit Data">
                            ...
                            <tr>
                                <td colspan="5" class="px-6 py-12 text-center">
                                    <div class="flex flex-col items-center justify-center">
                                        ...
                                        <h3 class="text-slate-500 font-medium">Belum ada data obat</h3>
                                        <p class="text-slate-400 text-sm mt-1">Silakan input obat baru ke inventaris.</p>
                                    </div>
                                </td>
                            </tr>
                            @endforelse

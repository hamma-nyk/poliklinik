    ...
    <x-slot name="header">
        <div class="flex flex-col md:flex-row justify-between items-center">
            ...
            <div class="hidden md:flex items-center text-sm text-slate-500 mt-2 md:mt-0 dark:text-slate-400">
                ...
                <span class="font-semibold text-slate-900 dark:text-slate-50">Obat</span>
            </div>
        </div>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="rounded-xl border border-slate-200 bg-white text-slate-950 shadow dark:border-slate-800 dark:bg-slate-950 dark:text-slate-50 p-4 flex flex-col md:flex-row justify-between items-center gap-4">
                ...
                <div class="w-full md:w-auto">
                    <a href="{{ route('inventory.medicines.create') }}" class="inline-flex items-center justify-center whitespace-nowrap rounded-md text-sm font-medium transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-slate-950 disabled:pointer-events-none disabled:opacity-50 bg-slate-900 text-slate-50 shadow hover:bg-slate-900/90 h-9 px-4 py-2 dark:bg-slate-50 dark:text-slate-900 dark:hover:bg-slate-50/90 w-full md:w-auto">
                    ...
                    <table class="w-full caption-bottom text-sm">
                        <thead class="[&_tr]:border-b">
                            <tr class="border-b transition-colors hover:bg-slate-100/50 dark:border-slate-800 dark:hover:bg-slate-800/50">
                                <th class="h-12 px-4 text-left align-middle font-medium text-slate-500 dark:text-slate-400">Detail Obat</th>
                                <th class="h-12 px-4 text-left align-middle font-medium text-slate-500 dark:text-slate-400">Satuan</th>
                                <!-- <th class="h-12 px-4 text-left align-middle font-medium text-slate-500 dark:text-slate-400">Harga Jual</th> -->
                                <th class="h-12 px-4 text-center align-middle font-medium text-slate-500 dark:text-slate-400">Stok</th>
                            ...
                            <tr class="border-b transition-colors hover:bg-slate-100/50 dark:border-slate-800 dark:hover:bg-slate-800/50 group">
                                <td class="p-4 align-middle">
                                    <div class="flex items-center">
                                        <div class="flex h-9 w-9 items-center justify-center rounded-md border border-slate-200 bg-slate-50 text-xs font-medium dark:border-slate-800 dark:bg-slate-900">
                                            {{ substr($medicine->name, 0, 1) }}
                                        </div>
                                        <div class="ml-4 grid gap-0.5">
                                            <div class="font-medium">{{ $medicine->name }}</div>
                                            <div class="text-xs text-slate-500 dark:text-slate-400">{{ $medicine->code }}</div>
                                        </div>
                                    </div>
                                </td>

                                <td class="p-4 align-middle">
                                    <div class="inline-flex items-center rounded-md border border-transparent bg-slate-100 text-slate-900 px-2.5 py-0.5 text-xs font-semibold dark:bg-slate-800 dark:text-slate-50">
                                        {{ $medicine->unit }}
                                    </div>
                                </td>
                                ...
                                <td class="p-4 align-middle text-center">
                                    @if($medicine->current_stock <= 5)
                                        <span class="inline-flex items-center rounded-md border border-transparent bg-destructive/10 text-destructive px-2.5 py-0.5 text-xs font-semibold animate-pulse">
                                ...
                                <td class="p-4 align-middle text-right text-sm">
                                    <div class="flex justify-end gap-1">
                                        ...
                                        <form action="{{ route('inventory.medicines.destroy', $medicine->id) }}" method="POST" onsubmit="return confirm('Hapus obat ini?');">
                                            @csrf @method('DELETE')
                                            <button type="submit" class="inline-flex items-center justify-center whitespace-nowrap rounded-md text-sm font-medium transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-slate-950 disabled:pointer-events-none disabled:opacity-50 border border-slate-200 bg-white shadow-sm hover:bg-red-100 hover:text-red-600 h-8 w-8 dark:border-slate-800 dark:bg-slate-950 dark:hover:bg-red-900/50 dark:hover:text-red-500" title="Hapus">
                            ...
                            <tr>
                                <td colspan="5" class="p-4 align-middle text-center">
                                    <div class="flex flex-col items-center justify-center py-6">
                                        <div class="bg-slate-50 dark:bg-slate-900 p-4 rounded-full mb-3 text-slate-500 dark:text-slate-400">
                                            <svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19.428 15.428a2 2 0 00-1.022-.547l-2.384-.477a6 6 0 00-3.86.517l-.318.158a6 6 0 01-3.86.517L6.05 15.21a2 2 0 00-1.806.547M8 4h8l-1 1v5.172a2 2 0 00.586 1.414l5 5c1.26 1.26.367 3.414-1.415 3.414H4.828c-1.782 0-2.674-2.154-1.414-3.414l5-5A2 2 0 009 10.172V5L8 4z"></path></svg>
                                        </div>
                                        <h3 class="text-sm font-medium">Belum ada data obat</h3>
                                        <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">Stok obat masih kosong, silakan tambah item inventaris.</p>
                                    </div>
                                </td>
                            </tr>
                            @endforelse

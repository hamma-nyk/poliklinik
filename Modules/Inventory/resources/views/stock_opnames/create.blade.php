<x-app-layout title="Input Stok Opname">
    <x-slot name="header">
        <div class="flex flex-col md:flex-row justify-between items-center gap-4">
            ...
            <a href="{{ route('inventory.stock-opnames.index') }}" class="inline-flex items-center justify-center whitespace-nowrap rounded-md text-sm font-medium transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-slate-950 disabled:pointer-events-none disabled:opacity-50 border border-slate-200 bg-white shadow-sm hover:bg-slate-100 hover:text-slate-900 h-9 px-4 py-2 dark:border-slate-800 dark:bg-slate-950 dark:hover:bg-slate-800 dark:hover:text-slate-50">
    ...
    <div class="py-6 flex-1 space-y-4">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <form action="{{ route('inventory.stock-opnames.store') }}" method="POST">
                ...
                {{-- Form Administrasi --}}
                <div class="rounded-xl border border-slate-200 bg-white text-slate-950 shadow dark:border-slate-800 dark:bg-slate-950 dark:text-slate-50 p-6 mb-8 transition-all">
                ...
                {{-- Tabel Opname --}}
                <div class="rounded-xl border border-slate-200 bg-white text-slate-950 shadow dark:border-slate-800 dark:bg-slate-950 dark:text-slate-50 overflow-hidden mb-8 transition-all">
                        ...
                        <table class="w-full caption-bottom text-sm">
                            <thead class="[&_tr]:border-b">
                                <tr class="border-b transition-colors hover:bg-slate-100/50 dark:border-slate-800 dark:hover:bg-slate-800/50 text-[11px] font-bold uppercase tracking-widest text-slate-500 dark:text-slate-400">
                                    <th class="h-12 px-4 text-left align-middle font-medium text-slate-500 dark:text-slate-400">Item Obat / Logistik</th>
                                    ...
                                    <th class="h-12 px-4 text-left align-middle font-medium text-slate-500 dark:text-slate-400">Keterangan</th>
                                </tr>
                            </thead>
                            <tbody class="[&_tr:last-child]:border-0">
                                ...
                                <tr class="border-b transition-colors hover:bg-slate-100/50 dark:border-slate-800 dark:hover:bg-slate-800/50 group">
                                    <td class="p-4 align-middle">
                                        ...
                                        <div class="text-[10px] text-slate-500 dark:text-slate-400 font-mono mt-0.5 tracking-tighter uppercase">{{ $med->code }}</div>
                                    </td>
                                    ...
                                    <td class="p-4 align-middle text-center">
                                        <input type="number" name="items[{{ $med->id }}][physical_stock]" 
                                            ...
                                            class="physical-input flex h-8 w-20 text-center mx-auto rounded-md border border-slate-200 bg-transparent px-3 py-1 text-sm shadow-sm transition-colors file:border-0 file:bg-transparent file:text-sm file:font-medium placeholder:text-slate-500 focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-slate-950 disabled:cursor-not-allowed disabled:opacity-50 dark:border-slate-800 dark:placeholder:text-slate-400 dark:focus-visible:ring-slate-300 font-mono tabular-nums"
                                            min="0" required>
                                    </td>
                                    ...
                                    <td class="p-4 align-middle">
                                        <span class="px-2.5 py-1 bg-slate-100 dark:bg-slate-700 rounded-lg text-[10px] font-black text-slate-500 dark:text-slate-400 border border-slate-200 dark:border-slate-800 uppercase tracking-widest">
                                            {{ $med->unit }}
                                        </span>
                                    </td>
                ...
                {{-- Aksi --}}
                <div class="mt-8 pt-6 border-t border-slate-200 dark:border-slate-800 flex flex-col sm:flex-row justify-end gap-3 mb-12">
                    <p class="text-xs text-slate-400 dark:text-slate-500 italic mr-2 self-center">Pastikan semua data input sudah sesuai dengan fisik gudang.</p>
            ...
            function updateRowDiff(inputEl) {
                ...
                if (display) {
                    ...
                    if (diff > 0) {
                        ...
                        display.className = 'diff-display font-bold text-base text-slate-300 dark:text-slate-600';
                    }
                }
            }
